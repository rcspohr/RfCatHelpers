...
def ConfigureD(d):
    ...
    if results.verbose == True:
        ...
        print(f" [-] Packet Length: {keyLen}")
        print(f" [-] Baud Rate: {baudRate}")
        print("[-] ---------------------------------")


parser = argparse.ArgumentParser(description="Simple program to scan for PWM OOK codes")
parser.add_argument(
    "-fa",
    action="store",
    default="433000000",
    dest="startFreq",
    help="Frequency to start scan at, defaults to 433000000",
    type=long,
)
parser.add_argument(
    "-fb",
    ...
    default="434000000",
    dest="endFreq",
    help="Frequency to end scan at, defaults to 434000000",
    type=long,
)
parser.add_argument(
    "-fs",
    ...
    default="50000",
    dest="stepFreq",
    help="Frequency step for scanning, defaults to 50000",
    type=long,
)
parser.add_argument(
    "-ft",
    ...
    default="1000",
    dest="timeStepFreq",
    help="Frequency step delay, defaults to 1000 milliseconds",
    type=long,
)
parser.add_argument(
    "-vv", action="store_true", dest="verbose", default=False, help="Verbose output"
)
parser.add_argument(
    "-a",
    action="store_true",
    default=False,
    help="Automatically replay after collecting",
)
parser.add_argument(
    "-br",
    action="store",
    dest="baudRate",
    default=4800,
    help="Baudrate to transmit at, defaults to 4800",
    type=int,
)
parser.add_argument(
    "-r",
    ...
    dest="repeat",
    default=15,
    help="Amount of times to repeat when transmitting, defaults to 15",
    type=int,
)
parser.add_argument(
    "-p",
    ...
    dest="paddingZeros",
    default=15,
    help="Amount of repeated zeros to search for when looking for patterns",
    type=int,
)
...
parser.add_argument(
    "-ms",
    action="store",
    dest="minimumStrength",
    default=-80,
    help="Minimum strength, defaults to -80",
    type=int,
)
parser.add_argument(
    "-ln",
    ...
    dest="lockNum",
    default=5,
    help="Minimum `codes` to receive before locking",
    type=int,
)
parser.add_argument(
    "-rp",
    ...
    dest="replayKey",
    default=True,
    help="Replay most common code automatically, default true",
)
...
print(
    f"Scanning for AM/OOK Remotes... Press {BOLD}{WHITE}<enter>{ENDC} to stop and {BOLD}{WHITE} any key{ENDC} to continue\n"
)
...
def showStatus():
    print(f"\r{BOLD}{ENDC}[ {BOLD}{YELLOW}")
    print(spinner.next())
    ...
    print(
        f"{ENDC} ] Freq: [ {LIGHTBLUE}{str(currFreq)}{ENDC} ] Strength [ {YELLOW}{str(strength)}{ENDC} ] Signals Found: [ {GREEN}{sigFound}{ENDC} ]"
    )
    if lockedFreq == True:
        print(f"{ENDC}{RED}[!FREQ LOCKED!]{ENDC}")
    # else:
    # 	sys.stdout.write(" " * 30)
    # sys.stdout.write(" " * 10)
    # yes, i know, icky!
    ...
    try:
        ...
        if sorted_lens and sorted_lens[0][0] > 0 and sorted_lens[0][0] < 400:
            ...
            for s in possibleStrings:
                ...
                if (
                    len(allstrings[currFreq]) > results.lockNum
                ) and lockOnSignal == True:
                    lockedFreq = True
...
if results.verbose == True:
    ...
    for sK in sortedKeys:
        ...
        print(f"{num}: {str(sK)} - Num signals Found: {currLen}")
        print("-----------------------------------------------")
...
if results.replayKey == True and len(sortedKeys) > 0:
    ...
    while (var < 0) or (var > len(sortedKeys)):
        try:
            var = int(input("Please enter key to use for replaying: "))
        except ValueError:
            pass
    ...
    if len(sorted_lens) > 0:
        ...
        print(f"[+] Transmitting key: {repeatNum} times")
        barSize = 50

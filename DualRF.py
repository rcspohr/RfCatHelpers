...
def startJam(rfcatInstance,rfcatjamFreq):
	...
	print(f"[!] Jamming at {str(rfcatjamFreq)}")
...
parser = argparse.ArgumentParser(description='Application to use two RfCat compatible devices to scan for a signal and jam it')
...
print("----------------------------------------")
print(f"(i) Start: {str(baseFreq)}")
print(f"(i) End: {str((baseFreq + (count*inc)))}")
print(f"(i) Min dBm: {str(rssi_minimum_for_signal)}")

d = RfCat(idx=0)
configure(d)
d.setModeRX()

input("Press Enter to setup Jammer")
d2 = RfCat(idx=1)
configure(d2)
		...
		if(jamming == False):
			...
			if (highestVal >= strongestSignal):
				...
				if(jamming == False):
					...
					print("[-] Stopping scan on scanner rfcat")
				jamming = True
